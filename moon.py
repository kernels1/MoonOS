...
def initialize_system():
    ...
    lock_file = os.path.join(ROOT_DIR, ".moon_boot_lock")
...
checks = [
    ...
    ("Verifying MoonDrive Integrity...", lambda: os.path.exists(os.path.join(ROOT_DIR, "MoonDrive")) or (os.makedirs(os.path.join(ROOT_DIR, "MoonDrive")) or True)),
        ...
        while True:
            try:
                ...
                if cmd == "shutdown":
                    ...
                    total, used, free = shutil.disk_usage(self.fs.drive_path)
                    ...
                    t = Table(title=f"Interface: {self.wifi.interface}")
                    t.add_row("Status", status)
                    t.add_row("SSID", str(self.wifi.connected_ssid))
                    t.add_row("IP", str(self.wifi.local_ip))
                    t.add_row("MAC", str(self.wifi.mac_address))
                    console.print(t)
                    ...
                    app_path = self.fs.get_full_path(os.path.join("apps", app_name))
