...
def initialize_system():
    needed = ["asciimatics", "cv2", "rich", "bs4", "PyQt5", "PyQt5.QtWebEngineWidgets", "pywifi"]
    missing = []
    ...
    if missing:
        print("\n" + "--- MOON-OS KERNEL UPDATE ---".center(80))
        pip_map = {
            "cv2": "opencv-python",
            "bs4": "beautifulsoup4",
        ...
        libraries = ["asciimatics", "opencv-python", "rich", "playwright", "beautifulsoup4", "PyQt5", "PyQtWebEngine"]

        for lib in libraries:
            print(f"[+] Setting up: {lib}...")
            subprocess.check_call([sys.executable, "-m", "pip", "install", lib, "--quiet"])

        print("\n" + "SYSTEM READY. REBOOTING TERMINAL...".center(80))
        time.sleep(2)

        current_script = os.path.abspath(__file__)
        subprocess.Popen(f'start "MoonOS Terminal" "{sys.executable}" "{current_script}"', shell=True)
        sys.exit(0)
...
class MoonEditor:
    ...
    def render(self):
        ...
        return Panel("\n".join(content), title=f"Editing: {self.path}", subtitle="CTRL+S: Save | ESC: Exit")
...
if __name__ == "__main__":
    ...
    if "--kernel" in sys.argv:
        try:
            ...
            status = kernel.run()
            if status == "REBOOT":
                subprocess.Popen(f'start "MoonOS Terminal" "{sys.executable}" "{current_script}" --kernel', shell=True)
                sys.exit()
        except Exception as e:
...
class MoonFS:
    ...
    def get_full_path(self, filename=""):
        base = os.path.join(os.getcwd(), self.drive, self.current_dir)
